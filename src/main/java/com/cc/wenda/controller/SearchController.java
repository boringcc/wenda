    ...
    public String search(Model model, @RequestParam("q") String keyword,
        ...
        try {
            ...
            for (Question question : questionList) {
                ...
                vo.set("question", q);
                vo.set("score",q.getScore());
                vo.set("followCount", followService.getFollowerCount(EntityType.ENTITY_QUESTION, question.getId()));
                vo.set("user", userService.getUser(q.getUserId()));
                vos.add(vo);
            }
            //重写vos.sort的Comparator方法即可，按照问题的分值来排序
            Collections.sort(vos, new Comparator<ViewObject>(){
                @Override
                public int compare(ViewObject c1, ViewObject c2) {
                    if((Double)c1.get("score") > (Double) c2.get("score")){
                        return -1;
                    }else if ((Double)c1.get("score") < (Double) c2.get("score")){
                        return 1;
                    }else {
                        return 1;
                    }
